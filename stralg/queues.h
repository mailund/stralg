...
static inline void init_index_queue(struct index_queue *queue)
{
    init_queue(queue);
}
static inline void dealloc_index_queue(struct index_queue *queue)
{
    dealloc_queue(struct index_linked_list, queue);
}
static inline struct index_queue *alloc_index_queue(void) {
    alloc_queue(struct index_queue);
}
static inline void free_index_queue(struct index_queue *queue) {
...
static inline void init_pointer_queue(struct pointer_queue *queue)
{
    init_queue(queue);
}
static inline void dealloc_pointer_queue(struct pointer_queue *queue)
{
    dealloc_queue(struct pointer_linked_list, queue);
}
static inline struct pointer_queue *alloc_pointer_queue(void) {
    alloc_queue(struct pointer_queue);
}
static inline void free_pointer_queue(struct pointer_queue *queue) {
    free_queue(struct pointer_linked_list, queue);
}

static inline bool is_pointer_queue_empty(const struct pointer_queue *queue) {
    return is_queue_empty(queue);
}
static inline void *pointer_queue_front(const struct pointer_queue *queue) {
    assert(queue->front != 0);
    return queue->front->data;
}

static inline void enqueue_pointer(struct pointer_queue *queue, void *pointer) {
    enqueue(struct pointer_linked_list, new_pointer_link, queue, pointer);
}
static inline void dequeue_pointer(struct pointer_queue *queue) {
